...
- A probe waits up to 30 seconds for its answer, so an upstream that silently drops queries instead of refusing them counts as failing rather than being waited on forever. Raising `timeout` under **Upstream DNS Servers** beyond 30 seconds does not extend this — no client on your network would wait that long for a lookup either.
