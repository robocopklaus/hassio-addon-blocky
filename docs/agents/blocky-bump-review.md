...
2. **Directed changelog scan (semantic truth, human judgement).** Read the upstream release notes, but *directed*: cross each entry against the **two surfaces the add-on is coupled to**. An entry that touches neither (e.g. an internal dnssec resolution fix) is out of our scope and passes without further work. An entry that touches either gets classified below.

   - **The config keys `blocky.gtpl` emits.** As of this writing that set is `upstreams`, `bootstrapDns`, `filtering`, `fqdnOnly`, `customDNS`, `conditional`, `ecs`, `dns64`, `rebindingProtection`, `clientLookup`, `blocking`, `caching`, `redis`, `prometheus`, `queryLog`, `ports`, `http3`, `log`, and the TLS/cert fields — but the template is the authoritative source (`grep -nE '^[a-zA-Z].*:' blocky/rootfs/usr/share/tempio/blocky.gtpl`), so re-derive it rather than trusting this list.
   - **The HTTP API endpoints and response shapes the add-on consumes.** `render-test` cannot see these: it validates rendered YAML, and no golden contains an API response, so an endpoint that is removed, renamed, or changes its response shape passes the gate of record silently. As of this writing: `/api/blocking/status` (the `watchdog` URL in `blocky/config.yaml`, consumed by the Supervisor) and `POST /api/query` with its `responseType`/`returnCode` fields and 5xx-on-chain-error behaviour (the upstream resolution probe, ADR-0012). Re-derive rather than trusting this list: `grep -rn '/api/' blocky/config.yaml blocky/rootfs`.

## Classifying a changelog entry that touches a coupled surface
